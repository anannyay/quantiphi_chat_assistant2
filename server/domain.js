...
export const messageSchema = z.object({ prompt: z.string().trim().min(1).max(8000), tone: z.enum(['professional', 'casual', 'concise']) }).strict();
...
export function contextFor(messages) {
  ...
  return messages.filter(m => m.status === 'complete').slice(-24).map(({ role, content }) => ({ role, content }));
}
