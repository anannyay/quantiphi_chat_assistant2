...
if (mode === 'live' && (!process.env.OPENAI_API_KEY || !process.env.MONGODB_URI))
  throw new Error('Live mode requires OPENAI_API_KEY and MONGODB_URI. Configure .env first.');
const model = process.env.OPENAI_MODEL || 'gpt-4.1-mini';
const repository = await (
  mode === 'live'
    ? new MongoRepository(process.env.MONGODB_URI, process.env.MONGODB_DB || 'cadence')
    : new FileRepository()
).init();
...
const provider =
  mode === 'live' ? new OpenAIProvider(process.env.OPENAI_API_KEY, model) : new DemoProvider();
const server = createApp({ repository, provider, mode, model }).listen(
  Number(process.env.PORT || 3001),
  process.env.HOST || '127.0.0.1',
  () =>
    console.log(
      `Cadence running on http://${process.env.HOST || '127.0.0.1'}:${process.env.PORT || 3001} (${mode})`,
    ),
);
