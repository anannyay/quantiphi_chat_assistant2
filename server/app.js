...
export function createApp({ repository, provider, mode, model }) {
  ...
  app.get('/api/health', (_req, res) =>
    res.json({
      status: 'ok',
      mode,
      model: mode === 'demo' ? 'Scripted demo' : model,
      storage: mode === 'demo' ? 'Local JSON' : 'MongoDB',
    }),
  );
  app.get('/api/conversations', async (_req, res) => res.json(await repository.list()));
  app.post('/api/conversations', async (_req, res) =>
    res.status(201).json(await repository.save(newConversation())),
  );
  async function find(id) {
    idSchema.parse(id);
    const c = await repository.get(id);
  ...
  app.post('/api/conversations/:id/messages', async (req, res) => {
    ...
    if (active.has(req.params.id))
      throw new HttpError(409, 'This conversation is already generating a reply.');
    active.add(req.params.id);
    const controller = new AbortController();
    let conversation, assistant;
    ...
    try {
      ...
      if (conversation.messages.length >= 200)
        throw new HttpError(409, 'This thread is full. Start a new conversation.');
