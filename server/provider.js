...
export class OpenAIProvider {
  constructor(apiKey, model) { this.client = new OpenAI({ apiKey, maxRetries: 1, timeout: 60000 }); this.model = model; }
  async *stream(messages, tone, signal) {
    const stream = await this.client.responses.create({ model: this.model, instructions: instructionsFor(tone), input: contextFor(messages), stream: true, store: false, max_output_tokens: 1800 }, { signal });
    let complete = false;
    for await (const event of stream) {
      ...
      if (['error', 'response.failed', 'response.incomplete'].includes(event.type)) throw new Error('The AI could not finish this response.');
    }
...
export class DemoProvider {
  async *stream(messages, tone, signal) {
    const topic = messages.at(-1).content.replace(/[\r\n]/g, ' ').slice(0, 160);
    const text = {
    ...
    for (const chunk of text.match(/.{1,5}|\n/g)) { await delay(12, undefined, { signal }); yield chunk; }
  }
}
