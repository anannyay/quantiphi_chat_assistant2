...
export async function readEvents(response, onEvent) {
  if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.error || 'Unable to send message.'); }
  ...
  try {
    ...
    if (!completed) throw new Error('Connection interrupted. Your partial response may have been saved.');
